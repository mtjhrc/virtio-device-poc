...
//! A simple framework to run a vhost-user-user backend service.
...
mod backend;
pub use self::backend::{VhostUserDevice, VhostUserDeviceImplementer};
...
#[derive(Debug)]
/// Errors related to vhost-user-user daemon.
pub enum Error {
    /// Failed to create a new vhost-user-user handler.
    NewVhostUserHandler(VhostUserHandlerError),
    /// Failed creating vhost-user-user backend listener.
    CreateBackendListener(VhostUserError),
    /// Failed creating vhost-user-user backend handler.
    ...
    /// Failed handling a vhost-user-user request.
...
impl Display for Error {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        match self {
            Error::NewVhostUserHandler(e) => {
                write!(f, "cannot create vhost-user user handler: {}", e)
            }
            ...
            Error::CreateVhostUserListener(e) => {
                write!(f, "cannot create vhost-user-user listener: {}", e)
            }
...
/// Result of vhost-user-user daemon operations.
pub type Result<T> = std::result::Result<T, Error>;

/// Implement a simple framework to run a vhost-user-user service daemon.
///
/// This structure is the public API the backend is allowed to interact with in order to run
/// a fully functional vhost-user-user daemon.
pub struct VhostUserDaemon<T: VhostUserDevice> {
    name: String,
    ...
    T::Bitmap: BitmapReplace + NewBitmap + Clone + Send + Sync,
{
    ...
    /// Connect to the vhost-user-user socket and run a dedicated thread handling
    ...
    /// Listen to the vhost-user-user socket and run a dedicated thread handling all requests coming
    /// through this socket.
    ///
    ...
    /// Wait for the thread handling the vhost-user-user socket connection to terminate.
    ///
    /// *Note:* A convenience function [VhostUserDaemon::serve] exists that
    /// may be a better option than this for simple use-cases.
